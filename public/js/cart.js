...
$('.block2-btn-addcart').click(function(){
    ...
    var quantity = 1;
    quickAddCart(quantity, product_id);
});
...
function quickAddCart(quantity, product_id) {
    $.ajax({
        url: '/add-to-cart',
        method: 'post',
        data: {
...
function quickAddCartAjax(){
    var product_id = $('.block2-btn-addcart').attr('id').replace('add-cart-', '');
    var quantity = 1;
    $.ajax({
        url: '/add-to-cart',
        method: 'post',
